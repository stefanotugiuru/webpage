...
async function loadYouTubeVideos() {
  ...
  videoGrid.innerHTML = '<p style="text-align:center;color:#666;font-size:1.2rem;">Caricamento video...</p>';
  ...
  try {
    ...
    if (!data.items || data.items.length === 0) {
      videoGrid.innerHTML = `<p style="text-align:center;color:#666;">Nessun video trovato</p>`;
      return;
    }
    ...
    setTimeout(() => {
      document.querySelectorAll('.video-container').forEach((el) => el.classList.add('show'));
    }, 100);
  } catch (error) {
